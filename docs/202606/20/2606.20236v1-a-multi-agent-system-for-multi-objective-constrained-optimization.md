...
tldr: 在动态环境下，强化学习解决约束优化问题时，通常将目标和约束加权为单标量奖励，但权重手工调整难以适应非平稳变化。本文提出MAMO，采用多智能体强化学习将权重选择视为学习问题，解耦任务执行与目标设计。实验表明该方法能自动平衡目标优化与约束避免，提升鲁棒性。这是迈向自主RL约束优化方案的第一步。
source: arxiv
...
motivation: 手动选择奖励权重难以在动态环境中平衡目标优化与约束避免。
method: 提出MAMO，通过多智能体强化学习将权重选择建模为学习问题。
result: 自动学习权重，在动态环境下实现鲁棒的约束优化。
conclusion: MAMO为动态约束优化提供更自主、鲁棒的RL解决方案。
---

## 摘要
计算和网络系统中的许多决策问题可以自然地表述为在性能约束下的成本最小化问题。在动态环境中，强化学习通常用于在运行时解决这类问题，方法是通过加权惩罚项将成本和约束违反嵌入到单个标量奖励中，遵循拉格朗日启发式的公式。然而，在这种背景下，所学策略的行为关键取决于这些权重的选择，而这些权重通常是手动选择的。这使得在优化主要目标和有效避免约束违反之间难以确定适当的权衡，特别是在非平稳环境中，它们的相对重要性可能会发生变化。本文提出了MAMO（多智能体多目标约束优化系统），一种通过多智能体强化学习来解决这种平衡问题的方法。MAMO通过将奖励权重的选择制定为学习问题，将任务执行与目标设计解耦，为动态环境中基于强化学习的约束优化问题提供了更自主、更稳健的解决方案的第一步。
