...
score: 8.0
evidence: 集中训练分散执行（CTDE）与混合奖励架构用于多智能体协调
tldr: "真实世界在线RL训练机器人操作面临样本效率低和多智能体非平稳问题。本文提出集中训练分散执行与混合奖励架构，将Q函数分解为任务与抓取头，并改进actor-critic目标以适配离散抓取分布。在比先前大5-25倍的域随机化下，成功率大幅提升，如网球放置60%→80%、方块搬运25%→95%。该框架提升样本效率与性能，可应对更广随机化范围。"
source: arxiv
...
motivation: 现有真实世界在线RL方法随机化范围小，且多智能体并行训练带来非平稳性，限制性能。
method: 提出CTDE与混合奖励架构统一框架，共享多头critic并将Q分解为任务与抓取头，重构目标以利用分解Q值并考虑离散抓取分布。
result: 在多种机器人任务中，成功率相对基线提升20-70个百分点，并完成基线始终失败的任务。
conclusion: 该框架显著改善样本效率和策略性能，可处理更大范围域随机化，具有实际应用价值。
---

## 摘要
真实世界在线强化学习（RL）提供了一种有前景的方法，可直接在物理世界中训练机器人操作策略，从而避免模拟到现实的差距，并通过人机交互实现持续的策略优化。近期方法已通过人工干预展示了样本高效的学习，但仍局限于较小的随机化范围，并面临同时训练多个智能体所引发的非平稳性挑战。为解决这些局限，我们引入了一个统一框架，结合集中训练与分散执行（CTDE）和混合奖励架构（HRA）。该框架使多个智能体能够共享一个集中式的多头评论家。评论家被分解为任务头与抓取头，分别对应稀疏任务奖励和基于势的抓取奖励。我们相应地重新制定了评论家与智能体的目标函数，以利用分解后的Q值，同时显式考虑离散夹持器策略的类别动作分布。实验结果表明，所提出的框架显著提升了样本效率和策略性能。我们在两个机械臂和一个模拟人形机器人上，对网球和香蕉拾放、锅复位及模拟方块搬移等任务进行了验证，并采用了维度级域随机化，其范围比先前工作大约5-25倍。与最先进的基线相比，我们的方法在网球拾放任务中将成功率从60%提升至80%，在香蕉拾放任务中从60%提升至90%，在模拟方块搬移中从25%提升至95%，同时还成功完成了基线始终失败的一项任务。视频和更多详情请访问我们的项目网站：https://hil-harc.github.io/。
